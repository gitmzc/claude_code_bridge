...
class WeztermBackend(TerminalBackend):
    ...
    def spawn_new_tab(self, cmd: str, cwd: str) -> str:
        """Spawn a new WezTerm tab (not window) in the current window."""
        args = [*self._cli_base_args(), "spawn"]
        force_wsl = os.environ.get("CCB_BACKEND_ENV", "").lower() == "wsl"
        use_wsl_launch = (is_wsl() and _is_windows_wezterm()) or (force_wsl and is_windows())
        ...
        try:
            ...
            raise RuntimeError(f"WezTerm spawn tab failed:\nCommand: {' '.join(args)}\nStderr: {e.stderr}") from e
